function [loglikelihood,nu,c2]=trainPCMixtureOneLMAPz(expNum,c2,nu,E,S)
S{1}=1;
[Nx,Ny]=size(E{1});
        loglikelihood=-inf;
        %expNum=expNum;
...
%             E{exper}  =   real(  ifft2( fft2(  (x-circshift(x, coord(exper,:)  )).^2   )  .* conj(HQ{1})   ));
        ...
        for emiter=1:200
            ...
            for exper=1:expNum
                ...
                Z{exper}  = gpuArray(  Z{exper}.*((gamma((nu{exper}+1)/2)/gamma((nu{exper})/2)) *(c2{exper}/nu{exper})^0.5*(1+c2{exper}*E{exper}/nu{exper} ).^(-(nu{exper}+1)/2 )).^2   );%
    ...
    %   PW{exper}=(Z{exper}+.1)/(1+.1);
       % pof{exper}=sum(sum(Z{exper}));
            ...
            for exper=1:expNum
                ...
                % PW{exper}=(Z{exper}+.0000000001)./(1+.000000001);
                    c2prev=c2{exper};
...
%                     end
                ...
                mysumA  =   mysumA+sum(sum(  Z{exper}.*log(A{exper}) - Z{exper}.*A{exper} ))/(Nx*Ny) ;
...
nuprev=nu{1};
            ...
            for exper=1:expNum
                c2{exper}  =   S{1}*gather( (Nx*Ny)  / lambda );
              nu{exper}=nuall;
            end
            ...
            %(nu{1:expNum})
            improv=abs(c2prev-c2{1}).^2+abs(nuall-nuprev).^2;
