...
for iter=1:5
     ...
     for exper=1:expNum
        ...
        %E1{exper}=(f-circshift(f,rw{2*exper-1})  ).^2;
         ...
         E   =  real(  ifft2( fft2( ( x_r-circshift(x_r, rw{2*exper-1})  ).^2  +  (x_g-circshift(x_g, rw{2*exper-1})  ).^2 + (x_b-circshift(x_b, rw{2*exper-1})  ).^2  + real(ifft2(fft2(q1).*(fft2(Qp))))) .* conj(HQ{1})  ));
...
%     end
    ...
    x_rprev=x_r;
    b = gpuArray(g1);
    tic;   [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_r ]=cgLanczos( @Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 150, 0.0000001); toc
   ...
   % g1 = imresize(g_g,decFactor);
    ...
    tic; [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_g ]=cgLanczos(@Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 150, 0.0000001); toc

    x_g=reshape(x,  Nx, Ny );
    D_g=reshape(D_g,  Nx, Ny );
    ...
    x_bprev=x_b;
    tic;[ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_b ]=cgLanczos(@Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 150, 0.0000001); toc
    x_b=reshape(x,  Nx, Ny );

    D_b=reshape(D_b,  Nx, Ny );
