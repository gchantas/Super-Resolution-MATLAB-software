...
%         end
    ...
    iw{1}=gpuArray((nb+1)./(nb+lambdab*abs(imresize(x_r,0.5)-g_r).^2));
    ...
    g1 = imresize( iw{1}.*g_r,2);
  
    x_rprev=x_r;
    b = gpuArray(g1);
    tic;x_r=reshape(cgLanczos(@Amat,gpuArray(zeros(NN,1)), b(:), iw, rw, 1, 0,100, 0.0000001), Nx, Ny); toc

   iw{1}=gpuArray((nb+1)./(nb+lambdab*abs(imresize(x_g,0.5)-g_g).^2));

    g1 = imresize(iw{1}.*g_g,2);
    ...
    tic;x_g=reshape(cgLanczos(@Amat,zeros(NN,1), b(:), iw, rw, 1, 0,100, 0.0000001), Nx, Ny); toc
    g1 = imresize(iw{1}.*g_b,2);

    iw{1}=gpuArray((nb+1)./(nb+lambdab*abs(imresize(x_b,0.5)-g_b).^2));


    b = gpuArray(g1);

    x_bprev=x_b;
    tic;x_b=reshape(cgLanczos(@Amat,zeros(NN,1), b(:), iw, rw, 1, 0,100, 0.0000001), Nx, Ny); toc
