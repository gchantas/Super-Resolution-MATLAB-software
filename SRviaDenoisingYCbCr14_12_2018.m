...
G_=4*padarray(g1,[extras extras],'symmetric');

G_=fft2(G_);



z_=4*padarray(g1,[extras extras],'symmetric');
Z_=fft2(z_);
%g3=imresize(g,2);
...
[x,alpha,Inv,HDDH]=StatSR(g2,g,1,ssigma,Nx,Ny,decFactor,f);
alpha
...
[fcorrsorted,I]=sort(fcorr(:),'descend');
...
%sigall=sqrt(abs(k1^2+l1^2));
           ...
           for exper=1:expNum
             ...
             %   E{exper} =  gpuArray(real(  ifft2( fft2( (f-circshift(f, rw{2*exper-1})  ).^2 ) .* conj(HQ{1}))  ));
...
%         q1=real(ifft2(fft2(qa))).^2;
      ...
      % Qe =  gpuArray(real(  ifft2( fft2( ( z-circshift(z, rw{2*exper-1})  ))))).^2;
          ...
          E =  gpuArray(real(  ifft2( fft2( ( x-circshift(x, rw{2*exper-1})  ).^2  + Qp+circshift(Qp, rw{2*exper-1})) .* conj(HQ{1})  )));
...
%toc
  ...
  %  xdown=real(fft2(Hcubic.*fft2(x)));
   % HC=multiplyBlockDiagMat(HcubBlockConj,multiplyBlockDiagMat(Inv,HcubBlock,decFactor*2),decFactor*2);

   
    
    %alpha= (Nx*Ny/decFactor^2)  /   (  sum(sum(  real(diagHHa.*Qp   )))*ssigma    +   sum(sum( (xdown-g).^2 )))   )
