...
for iter=1:25
     ...
     for exper=1:expNum
        ...
        %E1{exper}=(f-circshift(f,rw{2*exper-1})  ).^2;
         ...
         E   =  ( x-circshift(x, rw{2*exper-1})  );
         VE=real(ifft2( fft2(E) .* conj(HQ{1}) )) ;
         E=real(ifft2( fft2((E-VE).^2).* conj(HQ{1})  ));
...
%Imask3(Nx/2-bmaxX:Nx/2+1+bmaxX,Ny/2-bmaxY:Ny/2+1+bmaxY)=1;
 ...
 b = gpuArray2(g1);
    xHQ=real(ifft2(fft2( x).*conj(HQ{1})));
    ...
    for exper=1:expNum
     ...
     % B{exper}=real(fft2(fftshift(B{exper})));
      ...
      % disp(sprintf('c2(%d) %f', exper, c2{exper}));
       ...
       xb=B.*( xHQ-circshift(xHQ, rw{2*exper-1})  );
             b=b+ssigma* ( xb-circshift(xb, -rw{2*exper-1})  );
    end
