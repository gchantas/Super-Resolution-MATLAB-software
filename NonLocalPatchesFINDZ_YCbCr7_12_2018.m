%function [PSNR, ISNR, SSIMres]=NonLocalPatchesFINDZ_YCbCr7_12_2018(pathHR,pathLR)
...
[x,alpha,Inv]=StatSR(x,g,1,ssigma,Nx,Ny,decFactor,f);
...
% 
   ...
   % Qp=sum(sum( real(ifft2( hcubatrous ))  .*real(ifft2( conj(hcubatrous) )) ))  /    ssigma;
    ...
    tic;  x=conjGradients(x(:),@Amat_veryfast,b(:),iw,rw, 1000,10^(-8));toc
    %  tic;  x=PCconjGradients(x(:),@Amat_veryfast,b(:),iw,rw, Inv, 1000,10^(-8));toc
