%function [PSNR, ISNR, SSIMres]=SRviaDenoisingYCbCr7_11_2018(imNumber)
clear all;
myinput1=1;
imNumber=1;
%randn('seed',0);
...
% 
% %g3=imresize(g,2);
% g2 = myimresizeTranspose(g,Nx,Ny,Hcubic,decFactor,extras) ;
% G_=padarray(g2,[extras extras],'symmetric');
% 
% G_=fft2(G_)/ssigma;
...
% 
% 
% HcubBlock=makeDiagMatCells(Hcubic/sqrt(ssigma),Nx+2*extras,Ny+2*extras,decFactor*2,0);
% HcubBlockConj=makeDiagMatCells(conj(Hcubic)/(sqrt(ssigma)),Nx+2*extras,Ny+2*extras,decFactor*2,0);
% 
...
% tic
% Ia=makeDiagMatCells(alpha*ones(Nx+2*extras,Ny+2*extras),Nx+2*extras,Ny+2*extras,decFactor*2,0);%%Block diagonal identity matrix
% C2=addBlockMat(C,Ia,decFactor*2);
% Inv=invertBlockMatrix(C2,decFactor*2);
% 
% bVec=makeBlockVec(alpha*X_,Nx+2*extras,Ny+2*extras,decFactor,0);
...
% xSR=real(ifft2(vec));
% 
% norm(xSR(extras+1:Nx+extras,extras+1:Ny+extras)-x,'fro')/(Nx*Ny)
% toc
% 
...
%         q1=real(ifft2(fft2(qa))).^2;
      ...
      % Qe =  gpuArray(real(  ifft2( fft2( ( z-circshift(z, rw{2*exper-1})  ))))).^2;
...
%         end     
     ...
     % A=real(ifft2(  fft2(  (nu+S{1})./(nu+c2{exper}*Qe) )  .*   conj(HQ{1}) ));
      ...
      Z{exper}  =    PW{exper}.*exp(  J*log(A)-A.*(c2{exper}*E/2+nu/2)   );
...
%    g1 = myimresizeTranspose(g,Nx,Ny,Hcubic,decFactor,extras) ;
 ...
 tic
    xprev=x;
    x=cgLanczos( @EasySRAmat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 1000, 0.000000001); 
 toc
    x=reshape(x,  Nx, Ny );
 ...
 HF= hcubatrous.^2;%abs(Hcubic(1:Nx/2,1:Ny/2)).^2+abs(Hcubic(Nx/2+1:Nx,1:Ny/2)).^2+abs(Hcubic(1:Nx/2,Ny/2+1:Ny)).^2+abs(Hcubic(Nx/2+1:Nx,Ny/2+1:Ny)).^2;
  ...
  % alpha= Nx*Ny/ (TrH+ sum(sum(Qp)) + sum(sum( (x-z).^2 ) ) ) 
...
% Nx=Nx-2*extras;
    ...
    if norm(x-xprev,'fro')^2/(Nx*Ny)<10^(-7)
        break;
    end
end
