...
optcoord(Nx/2+1,Ny/2+1)=1;
            ...
            %Generalized Gaussian Filter Function
            sigblurx=sigall*0.5;
            sigblury=sigall*0.5;
             for i=1:Nx
                for j=1:Ny
   ...
   %Low pass filtering PSF
            sigblurx=sigall*0.5;
            sigblury=sigall*0.5;
             for i=1:Nx
                for j=1:Ny
                    hh2(i,j)=exp(-(abs(i-floor(Nx/2)-1).^3.0)*sigblurx).*exp(-(abs(j-floor(Ny/2)-1).^3.0)*sigblury);
                end
             end

            S{1}=1;
            hh2=S{1}*(hh2)/sum(sum(hh2));
...
% 
Fmask=zeros(Nx,Ny);
Fmask(Nx/2+1:2:Nx/2+1+70,Ny/2-36:2:Ny/2+36)=1;
...
%             %pof{exper}=1/expNum;
%             Qp=zeros(Nx,Ny);
%             rw{2*exper-1}=fft2(fftshift(hh-circshift(hh,coord(exper,:))));
%             PW{exper}=ones(Nx,Ny)/expNum;
%         end;
%         
%         
%   for exper=1:expNum
%          E{exper}   =  real( ifft2(  fft2(real(ifft2( fft2(g2).*rw{2*exper-1} )).^2  )   .* conj(HQ{1})  ));
...
% expNum=indx-1;
 ...
 %return;
clear E;
hh2(Nx/2+1,Ny/2+1)=1-hh2(Nx/2+1,Ny/2+1);
        ...
        for exper=1:expNum
            ...
            %pof{exper}=1/expNum;
            Qp=zeros(Nx,Ny);
            rw{2*exper-1}=fft2(fftshift(hh2-circshift(hh2,coord(exper,:))));
            PW{exper}=ones(Nx,Ny)/expNum;
        end;
      % x=g2(1:Nx,1:Ny);

        ssigma=10^(-7)/log(alpha+1);
...
for iter=1:10
     ...
     for exper=1:expNum
       ...
       Diag=real(ifft2( fft2( real(ifft2((rw{2*exper-1}))).^2).*fft2(Qp)));
         E   =  real( ifft2(  fft2(real(ifft2( fft2(x).*rw{2*exper-1} )).^2  )   .* conj(HQ{1})  ));
...
% 
   ...
   % Qp=sum(sum( real(ifft2( hcubatrous ))  .*real(ifft2( conj(hcubatrous) )) ))  /    ssigma;
    ...
    for exper=1:expNum
        ...
        Qp=Qp+real(ifft2(conj(fft2( ifft2((rw{2*exper-1})).^2)).*fft2(B{exper}) ));
    end
...
%         %iw{10}=ZeroIndX;
    ...
    PSNR=10*log10(  (Nx-2*(boundary-1)+1)*(Ny-2*(boundary-1)+1)  /  (  norm(f(boundary:Nx-boundary+1,boundary:Ny-boundary+1) - double((x(boundary:Nx-boundary+1,boundary:Ny-boundary+1))),   'fro'    )^2 ) )
