...
for iter=1:10
     ...
     for exper=1:expNum
      ...
      %  Z{exper}  =   exp(  log(c2{exper})/2+(S{1}/2+nu{exper}/2-1)*(log(A{exper})-log(J)+psi(J))-A{exper}.*(c2{exper}*E/2+nu{exper}/2) -log(gamma(nu{exper}/2))  +   log(nu{exper}/2)*(nu{exper}/2)  );
              Z{exper}  =  PW{exper}.*( (gamma((nu{exper}+S{1})/2)/gamma((nu{exper})/2)) *(c2{exper}/nu{exper})^0.5*(1+c2{exper}*E/nu{exper} ).^(-(nu{exper}+S{1})/2 ));%
    ...
    hcubatrous=hcubatrous(extras:Nx+extras-1,extras:Ny+extras-1);
   ...
   % Qp=sum(sum( real(ifft2( hcubatrous ))  .*real(ifft2( conj(hcubatrous) )) ))  /    ssigma;
    ...
    tic;   [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_r ]=cgLanczos( @Amat_fast,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 500, 10^(-12)); toc
