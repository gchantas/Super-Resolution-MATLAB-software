    ...
    for k1=-j1:round(sqrt(j1)):j1+round(sqrt(j1))-1
        for l1=-j1:round(sqrt(j1)):j1+round(sqrt(j1))-1
            ...
            %   if (abs(k1)==j1 | abs(l1)==j1)&(k1~=0 | l1~=0)
                ...
                sigall=1;
                %Gaussian Blurring Function
                sigblurx=sigall*0.15*0.5;
                sigblury=sigall*0.15*2;
                for i=1:Nx
                    for j=1:Ny
                        hh(i,j)=exp(-(abs(i-Nx/2-1).^2.7)*sigblurx).*exp(-(abs(j-Ny/2-1).^2.7)*sigblury);
                    end
                end
...
for iter=1:1
    ...
    for exper=1:expNum
        ...
        B{exper}= 0.25 *   c2{exper}  *   real(ifft2(  fft2(  B{exper}   )  .*  ( HQ{exper})   ))  /   S{kerNum};
...
%         end
    ...
    g1_ = real(ifft2( fft2(g_r) .* H1));%imresize(g_r,decFactor);
    g1 = g1_(1:dfac:Nx,1:dfac:Ny);
  
    x_rprev=x_r;
    b = gpuArray(g1);
    tic;   x_r=reshape( cgLanczos( @Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 130, 0.0000001), Nx, Ny ); toc

   % g1 = imresize(g_g,decFactor);
    g1_ = real(ifft2( fft2(g_g) .*H1));%imresize(g_r,decFactor);
    g1 = g1_(1:dfac:Nx,1:dfac:Ny);
    ...
    tic; x_g=reshape(cgLanczos(@Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 130, 0.0000001), Nx, Ny); toc

    %g1 = imresize(g_b,decFactor);

    g1_ = real(ifft2( fft2(g_b) .*H1));%imresize(g_r,decFactor);
    g1 = g1_(1:dfac:Nx,1:dfac:Ny);
    ...
    x_bprev=x_b;
    tic;x_b=reshape(cgLanczos(@Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 130, 0.0000001), Nx, Ny); toc
