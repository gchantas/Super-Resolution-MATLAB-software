function [PSNR, ISNR, SSIMres]=NonLocalPatchesFINDZ_YCbCr8_12_2019(pathHR,pathSR,pathLR,imNumber,decFactor)
...
%             sigblury=sigall*0.5;
%              for i=1:Nx
%                 for j=1:Ny%                     hh(i,j)=exp(-(abs(i-floor(Nx/2)-1).^3.0)*sigblurx).*exp(-(abs(j-floor(Ny/2)-1).^3.0)*sigblury);
%                 end
%              end
 %Gaussian Blurring Function
            sigblurx=0.25;
...
patchY=2.5;
        ...
        for exper=1:expNum
            ...
            if (coord(exper,1)>Nx/2)
                 lenX=mod(Nx-coord(exper,1),Nx);
            else
                 lenX=mod(coord(exper,1),Nx);
            end

            if (coord(exper,2)>Ny/2)
                 lenY=mod(Ny-coord(exper,2),Ny);
            else
                 lenY=mod(coord(exper,2),Ny);
            end
            ...
            nu{exper}=2*patchX^2*patchY^2/(2*patchX*patchY+max(patchX-abs(lenX),0)*max(patchY-abs(lenY),0) );

            %c2{exper}=;%/(2*patchX*patchY+max(patchX-abs(lenX),0)*max(patchY-abs(lenY),0) );
