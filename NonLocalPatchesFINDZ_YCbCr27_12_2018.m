...
%Changes after 30.28 db for Urban100
...
%         %iw{10}=ZeroIndX;
  ...
  %  end
    % tic;   [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_r ]=cgLanczos( @Amat_fast,x, b(:), iw, rw, 0, 0, 1000, 10^(-10)); toc
     ...
     tic;  x=conjGradients(x(:),@Amat_ultrafast,b(:),iw,rw, 1000,10^(-7));toc
...
iter
if norm(x-x_prev,'fro')/(Nx*Ny)<10^(-8)
    return;
end
