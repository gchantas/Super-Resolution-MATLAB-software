...
for j1=1:2:7
  ...
  %              for l1=-j1:j1
            ...
            sigblury=sigall*0.2;
             for i=1:Nx
                for j=1:Ny
                    hh(i,j)=exp(-(abs(i-floor(Nx/2)-1).^2.5)*sigblurx).*exp(-(abs(j-floor(Ny/2)-1).^2.5)*sigblury);
                end
             end
...
%         A{exper}  =   (nu+S{1})./(c2{exper}*E+nu)   ;
%         Z{exper}  =   exp(  J*log(A{exper})-A{exper}.*(c2{exper}*E/2+nu/2)   );
% 
% % 
...
for iter=1:10
     ...
     for exper=1:expNum
        ...
        Qe =  ( x-circshift(x, rw{2*exper-1}))  .^2;
        E   =  real(  ifft2( fft2( Qe  + Qp+circshift(Qp, rw{2*exper-1})) .* conj(HQ{1})  )) ;
      %  E   =  real(  ifft2( fft2( ( x_r-circshift(x_r, rw{2*exper-1})  ).^2  +  (x_g-circshift(x_g, rw{2*exper-1})  ).^2 + (x_b-circshift(x_b, rw{2*exper-1})  ).^2 ) .* conj(HQ{exper})  ));

        %E = (real(ifft2( fft2( ( x-circshift(x,rw{2*exper-1})  ).^2  + Qp) .*( HQ)  )));
       %A{exper}=real(ifft2(  fft2(  (nu+c2{exper}*Qe).^(-J) )  .*   conj(HQ{1}) ));
      
       A{exper}  =   real( ifft2 (     fft2(  (nu+S{1})./(c2{exper}*Qe+nu) ) .* conj(HQ{1}) ))  ;
     % A{exper}  =   (nu+S{1})./(c2{exper}*E+nu)   ;
       
     Z{exper} = real(ifft2(  fft2(  (nu+c2{exper}*Qe).^(-J) )  .*   conj(HQ{1}) ));
     %    Z{exper}  =   exp(  real(ifft2( fft2( J*(nu+S{1})./(c2{exper}*Qe+nu)).* conj(HQ{1}))) -A{exper}.*(c2{exper}*E/2+nu/2)   );
...
%         end
    ...
    tic;   [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_r ]=cgLanczos( @Amat_fast,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 400, 0.0000001); toc
