...
for j1=1:1:60
    ...
    if  fcorr(k1(indx),l1(indx))<fcorrMax*0.92
     break;
    end
...
for iter=1:12
    ...
    zenind=find(Zen>-log(1/expNum)/(Nx*Ny)-10^(-8));
    for exper=1:expNum
...
%Imask3(Nx/2-bmaxX:Nx/2+1+bmaxX,Ny/2-bmaxY:Ny/2+1+bmaxY)=1;
    for exper=1:expNum
      ...
      %   E   =  real(  ifft2( fft2( ( x-circshift(x, rw{2*exper-1})  ).^2  + Qp+circshift(Qp, rw{2*exper-1})) .* conj(HQ{1})  ));
       ...
       [Bind{exper}]=find(B{exper}<=0);

       B{exper}(Bind{exper})=0;
       %[k1, l1]= ind2sub([Nx,Ny],I(:)); 
