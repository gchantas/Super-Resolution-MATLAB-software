...
optcoord(Nx/2+1,Ny/2+1)=1;
            ...
            sigblury=sigall*0.8;
             for i=1:Nx
                for j=1:Ny
                    hh(i,j)=exp(-(abs(i-floor(Nx/2)-1).^3.3)*sigblurx).*exp(-(abs(j-floor(Ny/2)-1).^3.3)*sigblury);
                end
             end
     ...
     autocorrX=fftshift(real(ifft2(abs(fft2(x)).^2)));
     
     
     maxAC=autocorrX(Nx/2+1,Ny/2+1);
...
for j1=1:2:5
  ...
  %              for l1=-j1:j1
          ...
          corrCoeff=autocorrX(Nx/2+1+k1,Ny/2+1+l1);
               
%          if corrCoeff>maxAC*0.95
% 
%                     indx=indx+1;
...
for iter=1:10
     ...
     for exper=1:expNum
        ...
        %E1{exper}=(f-circshift(f,rw{2*exper-1})  ).^2;
         ...
         E   =  real(  ifft2( fft2( ( x-circshift(x, rw{2*exper-1})  ).^2  ) .* conj(HQ{1})  ));
...
%     Qp=0./Qp;
    ...
    tic;   [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_r ]=cgLanczos( @Amat_fast,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 450, 0.0000001); toc
