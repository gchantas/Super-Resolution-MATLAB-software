%function [PSNR, ISNR, SSIMres]=NonLocalPatchesFINDZ_YCbCr12_12_2018(pathHR,pathLR)
%Mean parameters inclusion

%Changes: make maxX and maxX depend on direction
...
for iter=1:10
     ...
     for exper=1:expNum
        ...
        E=real(  ifft2( fft2( ( x-circshift(x, rw{2*exper-1})  )  .* conj(HQ{1})  )));
      
        VE   =  real(  ifft2( fft2( ( x-circshift(x, rw{2*exper-1}) -E ).^2  + Qp+circshift(Qp, rw{2*exper-1})) .* conj(HQ{1})  ));
      ...
      %  E   =  real(  ifft2( fft2( ( x_r-circshift(x_r, rw{2*exper-1})  ).^2  +  (x_g-circshift(x_g, rw{2*exper-1})  ).^2 + (x_b-circshift(x_b, rw{2*exper-1})  ).^2 ) .* conj(HQ{exper})  ));
        ...
        A  =   (nu{exper}+S{1})./(c2{exper}*VE+nu{exper})   ;
        ...
        Z  =   ((gamma((nu{exper}+S{1})/2)/gamma((nu{exper})/2)) *(c2{exper}/nu{exper})^0.5*(1+c2{exper}*VE/nu{exper} ).^(-(nu{exper}+S{1})/2 ));%
...
BALL=zeros(Nx,Ny);
    ...
    for exper=1:expNum
     ...
     %  BE{exper}=  real(ifft2(  fft2(  BE{exper}  )  .*   ( HQ{1})   )).*(1./BALL)  /   S{1};
       E2{exper}=     real(ifft2(  fft2(  E2{exper}   )  .*   ( HQ{1})   ))  /   S{1};

        b  =   b  +   c2{exper}*(E2{exper}-circshift(E2{exper},-rw{2*exper-1}));
    end
...
%         %iw{10}=ZeroIndX;
    ...
    % tic;  x=PCconjGradients(x(:),@Amat_ultrafast,b(:),iw,rw, Inv, 1000,10^(-8));toc
