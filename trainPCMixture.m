function [loglikelihood,nu,c2]=trainPCMixture(expNum,c2,nu,coord,E,S)
...
%     end
            ...
            for exper=1:expNum
                ...
                %    Z{exper}  =   (Z{exper}+10^(-12)) ./ (ZALL+10^(-12));
                 %  PW{exper}=(Z{exper}+10.001)./(1+10.001);
                    c2prev=c2{exper};
                   c2{exper}  =  gather( sum(sum(  Z{exper}   ))  /   sum(sum(  Z{exper}.*A{exper}  .*E{exper}   )));
