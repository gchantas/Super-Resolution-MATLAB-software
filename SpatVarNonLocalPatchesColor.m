%function [PSNR, ISNR]=SpatVarNonLocalPatchesColor(imNumber,myinput1)
clear all
myinput1=1;
imNumber=801;
%randn('seed',0);
...
expNum=indx-1
  ...
  for i=1:N
      for j=1:N
          hh(i,j)=exp(-(abs(i-Nx/2-1).^2.5)*sigblurx).*exp(-(abs(j-Ny/2-1).^2.5)*sigblury);
      end
  end
...
%         B{exper}= 50*ones(Nx,Ny);  %*   c2{exper}  *   real(ifft2(  fft2(  B{exper}   )  .*   HQ{kerNum}   ))  /   S{kerNum};
...
%     x_rprev=x_r;
%     b = gpuArray(g1);
%     tic;x_r=reshape(cgLanczos(@Amat,gpuArray(zeros(NN,1)), b(:), iw, rw, 1, 0,80, 0.0000001), Nx, Ny); toc
% 
%     g1 = imresize(g_g,2);
% 
%     b = gpuArray(g1) ;
...
%     x_gprev=x_g;
% 
%     tic;x_g=reshape(cgLanczos(@Amat,zeros(NN,1), b(:), iw, rw, 1, 0,80, 0.0000001), Nx, Ny); toc
% 
%     g1 = imresize(g_b,2);
% 
%     b = gpuArray(g1);
% 
%     x_bprev=x_b;
%     tic;x_b=reshape(cgLanczos(@Amat,zeros(NN,1), b(:), iw, rw, 1, 0,80, 0.0000001), Nx, Ny); toc
% 
%     %figure,imagesc(x_b)
...
for iter=1:1
    ...
    for exper=1:expNum
        ...
        %Z{exper}(find(Z{exper}<0.05))=0.0;
        B{exper}=A{exper}.*Z{exper};
        B{exper}= 0.35 *   c2{exper}  *   real(ifft2(  fft2(  B{exper}   )  .*   HQ{kerNum}   ))  /   S{kerNum};
        rw{2*exper}  =   gpuArray(  ssigma*B{exper}   );
    end
...
%         end
    ...
    x_rprev=x_r;
    b = gpuArray(g1);
    tic;x_r=reshape(cgLanczos(@Amat,gpuArray(zeros(NN,1)), b(:), iw, rw, 1, 0,120, 0.0000001), Nx, Ny); toc
    ...
    tic;x_g=reshape(cgLanczos(@Amat,zeros(NN,1), b(:), iw, rw, 1, 0,120, 0.0000001), Nx, Ny); toc
    ...
    x_bprev=x_b;
    tic;x_b=reshape(cgLanczos(@Amat,zeros(NN,1), b(:), iw, rw, 1, 0,120, 0.0000001), Nx, Ny); toc
