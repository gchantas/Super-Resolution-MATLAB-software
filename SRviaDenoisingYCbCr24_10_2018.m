%function [PSNR, ISNR, SSIMres]=NonLocalPatchesYCbCr31_10_2018(imNumber)
clear all;
myinput1=1;
imNumber=1;
%randn('seed',0);
...
for iter=1:10
     ...
     for exper=1:expNum
        ...
        %E1{exper}=(f-circshift(f,rw{2*exper-1})  ).^2;
         ...
         E   =  real(  ifft2( fft2( ( x-circshift(x, rw{2*exper-1})  ).^2  + real(ifft2(conj(fft2(q1)).*(fft2(Qp))))) .* conj(HQ{1})  ));
...
%     end
    ...
    x_prev=x;
    b = gpuArray(g1);
    tic;   [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D_r ]=cgLanczos( @Amat,gpuArray(zeros(Nx,Ny)), b(:), iw, rw, 0, 0, 400, 0.0000001); toc
