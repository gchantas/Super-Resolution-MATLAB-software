...
for j1=1:1:11
    for k1=-j1:round(abs(j1)^.0):j1+round(abs(j1)^.0)-1
        for l1=-j1:round(abs(j1)^.0):j1+round(abs(j1)^.0)-1
      ...
      %  E   =  real(  ifft2( fft2( ( x_r-circshift(x_r, rw{2*exper-1})  ).^2  +  (x_g-circshift(x_g, rw{2*exper-1})  ).^2 + (x_b-circshift(x_b, rw{2*exper-1})  ).^2 ) .* conj(HQ{exper})  ));
        ...
        %for emiter=1:10
         ...
         %  ZexperZALL=Z{exper}./ZALL;
          ...
          %  ZALL=ZALL-Z{exper};
      %  end  
c2{exper}=Nx*Ny/sum(sum(A{exper}));
        ...
        prevlikelihood=loglikelihood_vec(exper);
        loglikelihood=0;
         ...
         for ex=1:exper
            ...
            loglikelihood  =   loglikelihood  +  sum(sum( -c2{ex}*A{ex}.*Z{ex}.*E{ex}./ZALL  +   Z{ex}.*log(A{ex}*c2{ex})./ZALL ));
         end
         
         
         
      %  loglikelihood_vec(exper)=loglikelihood;
         
         
         
        if loglikelihood<prevlikelihood
             ZALL=ZALL-Z{exper};
            indx=indx-1;
        else
                    loglikelihood_vec(exper+1)=loglikelihood;
