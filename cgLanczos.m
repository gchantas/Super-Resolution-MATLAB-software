function [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D, D2 ] = ...
           cgLanczos( A,x0, b,iw,rw, show, check, itnlim, rtol )
...
if check                    % See if A is symmetric.
  w1   = (1:n)';
  w2   = 1./w1;
  s    = w2'*LanczosxxxA( A,w1,iw,rw );
  t    = w1'*LanczosxxxA( A,w2,iw,rw );
  z    = abs(s-t);
...
if ~done                        % k = itn = 1 first time through
  while itn < itnlim
    ...
    %   v3     = (1/beta3)*p.
    %-----------------------------------------------------------------
    p     = LanczosxxxA( A,v,iw,rw );
    alpha = v'*p;                        % alpha = v'Av
...
%       for kl=1:P
%           D2{kl}     = real(ifft2( (fQ{kl}).* fft2( reshape(w,N,N)))).^2;
%       end
      ...
      D      = D + w.^2;
%       for kl=1:P
%           D2{kl}     =  D2{kl}+real(ifft2( (fQ{kl}).* fft2(reshape(w,N,N)))).^2;
%       end
...
function y = LanczosxxxA( A,x,iw,rw )
